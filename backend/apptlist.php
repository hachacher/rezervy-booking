...
if(!isset($rzvy_rolepermissions['rzvy_list_view']) && $rzvy_loginutype=='staff'){ ?>
...
<?php die(); } ?>
      ...
      <ol class="breadcrumb">
		...
		<div class="col-md-12">
			<a class="btn btn-sm pull-right" href="<?php echo SITE_URL; ?>backend/appointments.php"><i class="fa fa-list"></i> <?php if(isset($rzvy_translangArr['calendar_view'])){ echo $rzvy_translangArr['calendar_view']; }else{ echo $rzvy_defaultlang['calendar_view']; } ?></a>
		</div>
      </ol>
      <!-- DataTables Card-->
      <div class="card mb-3">
          ...
          <div class="table-responsive">
            <table class="display responsive nowrap" width="100%" id="rzvy_appt_list_table">
			  ...
			  <tbody>
				...
				$all_bookings_list = $obj_bookings->get_all_appointments_listview();
				...
				if(mysqli_num_rows($all_bookings_list)>0){
					while($booking = mysqli_fetch_assoc($all_bookings_list)){
						...
						?>
						<tr>
						  <td><?php if($staff_name != ""){ echo $staff_name; }else{ echo '<center><i class="fa fa-minus"></i></center>'; } ?></td>
