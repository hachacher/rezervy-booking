...
$rzvy_show_cancelled_appointments = $obj_settings->get_option('rzvy_show_cancelled_appointments');
...
if(isset($_SESSION["rzvy_staff_calendar"])){
	if(is_numeric($_SESSION["rzvy_staff_calendar"])){
		...
		if($rzvy_show_cancelled_appointments != "Y"){
			$all_appointments = $obj_bookings->get_all_appointments_without_cancelled();
		}else{
...
while($appointment = mysqli_fetch_array($all_appointments)){
	...
	if($customer_name != "" && $customer_name != " "){
		$cnames = " ".$label_with." ".$customer_name." ";
	}else{
		$cnames = "";
	}
	
	$booking_addons = array();
	$booking_addons_detail = '';
	if($appointment['addons']!=''){
		$addonsinfo = unserialize($appointment['addons']);
		foreach($addonsinfo as $addoninfo){
			$obj_bookings->addons = $addoninfo['id'];
			$addontitle = $obj_bookings->readone_addon_title();
			$booking_addons[] = $addontitle.'('.$addoninfo['qty'].')';
		}
		if(sizeof($booking_addons)>0){
			$booking_addons_detail = ','.implode(',',$booking_addons);
		}
	}
	...
	if($rzvy_book_with_datetime == "Y"){
		...
		$event_title = str_replace('Anonymous Customer','Client Anonim',$customer_name)." ".$label_on." ".date($rzvy_time_format, strtotime($appointment['booking_datetime']))." ".$label_to." ".date($rzvy_time_format, strtotime($appointment['booking_end_datetime']))." ".$label_for." ".$appointment['title'].$booking_addons_detail;
	}else{
		/* $event_title = $cnames.$appointment['title']." ".$staff_name; */
		$event_title = str_replace('Anonymous Customer','Client Anonim',$customer_name)." ".$label_for." ".$appointment['title'].$booking_addons_detail;
	}
